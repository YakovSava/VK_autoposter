...
# Импортируем плагины
from plugins.Error_handler_plugin import Error_handler
from plugins.logging_plugin import Async_logging
from plugins.binder_plugin import Binder
from plugins.Bot_plugin import Bot, Bot_timer
from plugins.image_plugin import ImageGenerator
from plugins.configuration.tokens import vk_token, group_id
...
class Group():
	...
	async def photo_post(self):
		photo_t = dict(await vk.api.photos.get_wall_upload_server(group_id = group_id))
		async with ClientSession() as session:
			photo_path = await generator.generate()
			images = ""
			#print(photo_path)
			for photos in photo_path:
				async with session.post(
						photo_t["upload_url"],
						data = {"photo": open(join("Only_image", photos), "rb")}
					) as resp:
						response = await resp.read()
				response_dictionary = eval(f"dict({response.decode()})")
				server = response_dictionary['server']
				photo = response_dictionary['photo']
				hash = response_dictionary['hash']
				
				photo_info = dict((await vk.api.photos.save_wall_photo(group_id = group_id,
								photo = photo,
								server = server,
								hash = hash))[0])
				photo_id = photo_info["id"]
				owner_id = photo_info["owner_id"]
				images += f"photo{owner_id}_{photo_id},"
		...
		for photos in photo_path:
			await generator.delete(photos)
		return dict(post_info)['post_id']
...
async def main():
	...
	while True: # Запускаем бота в бесконечном цикле
		...
		if counter == counter_stop['post_images']:
			postid = await group_bot.photo_post()
			await bot.remember_time_of_post(postid, parameters['post_live'])
