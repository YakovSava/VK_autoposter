...
from plugins.logging_plugin import Async_logging # Асинхронный логгинг
...
Что в нашем случае? Тут мы постоянно создаём новые корутины, поэтому ошибки RuntimeError быть не должно. Если же будет напишите мне пожалуйста.
	...
	async def get_json(self): # Получение JSON
		try:
			async with _open(join("plugins/configs", "posts.json"), "r", encoding='utf-8') as json_file:
			...
			return {} # Это для первого запуска