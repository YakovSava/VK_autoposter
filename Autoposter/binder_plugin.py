...
from logging_plugin import Async_logging # Импортируем плагин асинхронного логгирования
...
class Binder(): # Обьявляем класс
	...
	async def get_parameter(self):
		...
		async with _open('config.json', 'r', encoding='utf-8') as json_parameter_file:
	...
	async def save_folder_order_parameter(self, new:int):
		...
		async with _open('config.json', 'w', encoding='utf-8') as json_parameter_file:
	...
	async def get_video_json(self):
		...
		async with _open("videos.json", "r", encoding = "utf-8") as video_json:
	...
	async def save_video_id(self, id):
		if isinstance(id, int):
			...
			async with _open("videos.json", "w", encoding = "utf-8") as video_json:
